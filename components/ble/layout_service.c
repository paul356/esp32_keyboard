...
void layout_service_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_EXEC_WRITE_EVT:
        ...
        if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC)
        {
            ...
            ESP_LOGI(TAG, "Executing prepared writes for layout update data for conn_id: %d", param->exec_write.conn_id);

            // Get connection context for this specific connection
            connection_context_t* ctx = get_connection_context(param->exec_write.conn_id);
            if (ctx && ctx->layout_update_data.data_len > 0)
            {
                // Ensure null-termination for JSON processing
                ctx->layout_update_data.data[ctx->layout_update_data.data_len] = '\0';
            ...
            {
                ESP_LOGW(TAG, "No layout update data to execute for conn_id: %d", param->exec_write.conn_id);
            }
        }
        else
        {
            // Cancel prepared writes
            ESP_LOGI(TAG, "Cancelling prepared writes for layout update data for conn_id: %d", param->exec_write.conn_id);

            // Get connection context and reset buffer if writes are cancelled
            connection_context_t* ctx = get_connection_context(param->exec_write.conn_id);
            if (ctx) {
                ctx->layout_update_data.data_len = 0;
            }
